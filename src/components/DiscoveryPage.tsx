...
export function DiscoveryPage({ tab = 'recipes' }: DiscoveryPageProps) {
  ...
  useEffect(() => {
    loadRecipes({ tags: selectedTags, query: searchQuery });
    loadCookbooks({ query: searchQuery });
  }, [loadRecipes, loadCookbooks, selectedTags, searchQuery]);
  ...
  const handleTagClick = (tag: string) => {
    ...
    updateUrlParams(searchQuery, nextTags);
  };
  ...
  const handleSearchChange = (query: string) => {
    ...
    updateUrlParams(query, selectedTags);
  };
  ...
  const handleLoadMoreCookbooks = async () => {
    await loadMoreCookbooks({ query: searchQuery });
  };
