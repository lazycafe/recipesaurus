...
export const MEAL_PLAN_MAX_PUBLIC_RECIPES = 240;
...
export function summarizeRecipesForMealPlan(
  recipes: MealPlanRecipeContext[],
  limit = MEAL_PLAN_MAX_RECIPES
...
export function buildMealPlannerInstructions(): string {
  return [
    ...
    'After the user\'s own recipes, use relevant public Recipesaurus recipes. Mark public recipe matches as "From Recipesaurus" and use exact recipe titles from the JSON.',
    ...
    'If the saved collection and public Recipesaurus recipes do not contain enough appropriate recipes, use fewer recipes and explain the limitation in the prep note.',
    'For breakfast, brunch, snack, or dessert requests, use those categories only in the slots where they make sense.',
    'Do not suggest new recipe ideas, starter recipes, invented recipes, or recipes outside the provided JSON.',
    'Do not use "New idea". Use only "From your recipes" or "From Recipesaurus" recipe labels.',
  ...
  return JSON.stringify(
    {
      ...
      publicRecipes: summarizeRecipesForMealPlan(publicRecipes, MEAL_PLAN_MAX_PUBLIC_RECIPES),
    },
    null,
    2
  );
...
export function buildFallbackMealPlan(request: string, recipes: MealPlanRecipeContext[]): string {
  ...
  if (matchingRecipes.length === 0) {
    return [
      `Request: ${request}`,
      '',
      'No matching saved or public Recipesaurus recipes were available for this request.',
      '',
      'Prep note: Try broadening the request or saving a few public Recipesaurus recipes before asking for a cookbook-ready plan.',
    ].join('\n');
  }
  ...
  return [
    ...
    ...recipeLines,
    '',
    'Prep note: This plan only uses saved recipes and existing public Recipesaurus recipes, so add more public recipes if you want a broader mix.',
  ].join('\n');
}
