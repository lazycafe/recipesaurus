...
interface RecipeSourceSnapshot {
  id: string;
  title: string;
  description: string;
  ingredients: string[];
...
interface Cookbook {
  ...
  source_cookbook_snapshot?: string | null;
  is_public: number;
  created_at: number;
  updated_at: number;
}

interface CookbookSourceSnapshot {
  id: string;
  name: string;
  description?: string | null;
...
async function handleGetRecipes(request: Request, db: D1Database): Promise<Response> {
  ...
  const formattedRecipes = dedupeRecipeRows(recipes.results, user.id).map(r => formatPublicRecipe(r, user.id));
...
async function handleCreateRecipe(request: Request, db: D1Database): Promise<Response> {
  ...
  const now = Date.now();
  const sourceSnapshot = serializeSourceRecipeSnapshot(body.sourceRecipe);
  const sourceRecipeId = body.sourceRecipeId || body.sourceRecipe?.id || null;
  await db.prepare(`
...
async function handleGetCookbooks(request: Request, db: D1Database): Promise<Response> {
  ...
  `).bind(user.id).all<Cookbook & { recipe_count: number; source_recipe_ids?: string | null }>();
  ...
  const shared = await db.prepare(`
    ...
    GROUP BY c.id
    ORDER BY c.updated_at DESC
  `).bind(user.id).all<Cookbook & { recipe_count: number; owner_name: string; source_recipe_ids?: string | null }>();
...
async function handleCreateCookbook(request: Request, db: D1Database): Promise<Response> {
  ...
  const now = Date.now();
  const sourceSnapshot = serializeSourceCookbookSnapshot(body.sourceCookbook);
  const sourceCookbookId = body.sourceCookbookId || body.sourceCookbook?.id || null;

  await db.prepare(`
    INSERT INTO cookbooks (id, user_id, name, description, cover_image, is_system, system_type, source_cookbook_id, source_cookbook_snapshot, is_public, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).bind(
    cookbookId,
    user.id,
...
async function handleGetCookbook(request: Request, db: D1Database, cookbookId: string): Promise<Response> {
  ...
  const recipes = await db.prepare(`
    SELECT r.*, owner.name as owner_name, cr.added_by_user_id, added.name as added_by_user_name FROM recipes r
    JOIN cookbook_recipes cr ON r.id = cr.recipe_id
    LEFT JOIN users owner ON r.owner_id = owner.id
    LEFT JOIN users added ON cr.added_by_user_id = added.id
    WHERE cr.cookbook_id = ?
    ORDER BY cr.added_at DESC
  `).bind(cookbookId).all<Recipe & { owner_name: string | null; added_by_user_id: string; added_by_user_name: string | null }>();

  const formattedRecipes = recipes.results.map(r => ({
    ...formatPublicRecipe(r, user.id),
...
async function handleGetSharedCookbook(request: Request, db: D1Database, token: string): Promise<Response> {
  ...
  const recipes = await db.prepare(`
    ...
    WHERE cr.cookbook_id = ?
    ORDER BY cr.added_at DESC
  `).bind(link.cookbook_id).all<Recipe & { owner_name: string | null }>();

  const formattedRecipes = recipes.results.map(r => formatPublicRecipe(r));
...
function buildSourceRecipeSnapshot(recipe: Recipe & { owner_name?: string | null }): RecipeSourceSnapshot {
  return {
    id: recipe.id,
    title: recipe.title,
    description: recipe.description,
...
function buildSourceCookbookSnapshot(
  cookbook: Cookbook & { owner_name?: string | null },
  recipeIds: string[]
...
function isCookbookUnmodifiedFromSource(
  cookbook: Cookbook & { recipe_count?: number; source_recipe_ids?: string | null },
...
function formatCookbookInfo(
  cookbook: Cookbook & { recipe_count: number; owner_name?: string | null; source_recipe_ids?: string | null },
  isOwner: boolean
) {
  const sourceCookbook = parseSourceCookbookSnapshot(cookbook.source_cookbook_snapshot);
  ...
  return {
    ...
    updatedAt: cookbook.updated_at,
    isOwner,
    ownerName: isOwner ? undefined : cookbook.owner_name || undefined,
    sourceCookbookId: cookbook.source_cookbook_id || sourceCookbook?.id || null,
    sourceCookbook,
    sourceRecipeIds: parseSourceIdList(cookbook.source_recipe_ids),
  };
}
...
function formatPublicRecipe(
  ...
  const sourceRecipe = parseSourceRecipeSnapshot(recipe.source_recipe_snapshot);
  ...
  return {
    ...
    createdAt: recipe.created_at,
    sourceRecipeId: recipe.source_recipe_id || sourceRecipe?.id || null,
    sourceRecipe,
  };
}
...
async function handleDiscoverCookbooks(request: Request, db: D1Database): Promise<Response> {
  ...
  return jsonResponse(
    {
      cookbooks: cookbooks.results.map(cookbook => formatCookbookInfo(cookbook, user ? cookbook.user_id === user.id : false)),
      total: count?.count || 0,
    },
    200,
    corsHeaders(origin)
  ...
  return jsonResponse(
    {
      cookbook: formatCookbookInfo(
        ...
        user ? cookbook.user_id === user.id : false
      ),
      recipes: recipes.results.map(recipe => formatPublicRecipe(recipe, user?.id)),
    },
    200,
...
async function findExistingSavedCookbookId(
  ...
  ).all<Cookbook & { recipe_count: number; source_recipe_ids?: string | null }>();
...
async function handleDiscoverSaveCookbook(request: Request, db: D1Database, cookbookId: string): Promise<Response> {
  ...
  `)
    .bind(cookbookId)
    .first<Cookbook & { owner_name: string | null }>();

  if (!cookbook) {
    return errorResponse('Cookbook not found or not public', 404, origin);
  }
  if (cookbook.user_id === user.id) {
    return jsonResponse({ id: cookbook.id }, 200, corsHeaders(origin));
  }
  ...
  const now = Date.now();
  const sourceRecipeIds = (cookbookRecipes || []).map(recipe => recipe.id);
  const existingCookbookId = await findExistingSavedCookbookId(db, user.id, cookbook, sourceRecipeIds);
...
async function handleDiscoverSaveRecipe(request: Request, db: D1Database, recipeId: string): Promise<Response> {
  ...
  `)
    .bind(recipeId)
    .first<Recipe & { owner_name: string | null }>();
...
// Discover - remix a public recipe into an editable private copy
async function handleDiscoverRemixRecipe(request: Request, db: D1Database, recipeId: string): Promise<Response> {
  const user = await getSessionUser(request, db);
  const origin = request.headers.get('Origin');
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      // Discover - remix recipe route
      const discoverRemixRecipeMatch = path.match(/^\/api\/discover\/recipes\/([^/]+)\/remix$/);
      if (discoverRemixRecipeMatch && method === 'POST') {
        return handleDiscoverRemixRecipe(request, env.DB, discoverRemixRecipeMatch[1]);
      }
